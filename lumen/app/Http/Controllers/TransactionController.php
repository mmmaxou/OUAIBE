...
class TransactionController extends Controller {
  ...
  public function update(Request $request, $id) {
    ...
    if (!empty($request->get('dateTransaction')))
      $transaction->dateTransaction = $request->get('dateTransaction');
    if (!empty($request->get('shortDescription')))
      $transaction->shortDescription = $request->get('shortDescription');
    if (!empty($request->get('output')))
      $transaction->output = $request->get('output');
    if (!empty($request->get('input')))
      $transaction->input = $request->get('input');
    $transaction->save();
